...
#solver = PFTDPWSolver(criterion=PolyUCB(1.0)) #From Anka: Try this solver
...
r_history = Vector{Float16}()
u_history = Vector{Float16}()
for (s, a, o, r) in stepthrough(cbsimulator, basepolicy, "s,a,o,r", max_steps=100)
    println("State was $s,")
    println("action $a was taken,")
    println("received observation $o and reward $r")
    push!(i_history, s.i * 100)
